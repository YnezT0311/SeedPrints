...
def main(target_model_name, ref_model_name, args):
    ...
    suspected_model = AutoModelForCausalLM.from_pretrained(model_name, revision=target_model_name, trust_remote_code=True).eval()
    ref_model = AutoModelForCausalLM.from_pretrained(model_name, revision=ref_model_name,trust_remote_code=True).eval()
