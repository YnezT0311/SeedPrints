...
MODEL_ROOT_DIR = os.path.abspath(os.path.join(PROJECT_ROOT, '../../../..', 'model'))
...
def get_model_save_path(model_tag: str):
    ...
    model_path = os.path.join(MODEL_ROOT_DIR, model_tag)
    print(f"Model path: {model_path}")
    if not os.path.exists(model_path):
        print(f"Model path {model_path} should be a ckpt file.")
        model_tags = model_path.split('/')
        model_path = os.path.join(PROJECT_ROOT, model_tags[0], model_tags[-1]) # TODO: deal with the ckpt file
