...
def get_acts(statements, tokenizer, model, layers, device, token_pos=-1):
    ...
    for statement in tqdm(statements):
        ...
        for layer, hook in zip(layers, hooks):
            hs = hook.out[0] if isinstance(hook.out, (tuple, list)) else hook.out  # hidden states tensor
            if hs.dim() == 3:        # (batch, seq, hidden)
                vec = hs[0, token_pos, :]      # (hidden,)
            elif hs.dim() == 2:      # (seq, hidden)
                vec = hs[token_pos, :]         # (hidden,)
            else:
                raise ValueError(f"Unexpected shape: {hs.shape}")
            
            acts[layer].append(vec)
    
    # stack len(statements)'s activations
    for layer, act in acts.items():
        acts[layer] = torch.stack(act).float()
        print(f"Layer {layer} activations shape: {acts[layer].shape}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", default="llama-160M-fullset-finetune-code_stack-seed-1000")
    parser.add_argument("--layers", nargs='+',default=[-1], help="Layers to save embeddings from")
