...
SeedPrints takes a fundamentally different approach: it leverages random initialization biases as persistent, seed-dependent identifiers present even before training begins. We show that untrained models exhibit reproducible prediction biases induced by their initialization seed, and that these weak signals remain statistically detectable throughout training. Unlike prior techniques that fail during early pre-training, SeedPrints remains effective across all training stages — from initialization to large-scale pre-training and downstream adaptation. Initialization itself imprints a unique and persistent identity on LLMs, forming a true biometric-like fingerprint.
